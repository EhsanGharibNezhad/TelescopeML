...
if args.worker:
    w = KerasWorker(
                ...
                sleep_interval = 0.5, nameserver=ip_address,run_id='CNNtrain_2')
    
    w.run(background=False)
    exit(0)

# Start a nameserver (see example_1)
NS = hpns.NameServer(run_id='CNNtrain_2', host=ip_address, port=None)
NS.start()


# Run an optimizer (see example_2)

result_logger = hpres.json_result_logger(directory='/data2/ehsan_storage/telescopeML_project/outputs/bohb_outputs/out3', overwrite=True)
