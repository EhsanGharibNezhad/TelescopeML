...
class DataProcessorClassification:
    """
    Perform various tasks to process the datasets for machine learning methods, including classification:
    ...
    ----------
    flux_values : np.ndarray
        Feature arrays (input data).
    wavelength_names : List[str]
        Name of wavelength in micron.
    wavelength_values : np.ndarray
        Wavelength array in micron.
    output_values : np.ndarray
        Output variable array (classification labels).
    output_names : List[str]
        Name of the output variable.
    spectral_resolution : int, optional
        Resolution of the synthetic spectra used to generate the dataset.
    trained_ML_model : BaseEstimator, optional
        Machine learning model object from sklearn package.
    trained_ML_model_name : str, optional
    ...
    def __init__(
        ...
        self.trained_ML_model_name = trained_ML_model_name
        self.ml_method = ml_method
        self.LoadSave = LoadSave(trained_ML_model_name, ml_method)
    ...
    def split_train_test(self, test_size=0.1):
        ...
        self.X_test : array
            X test set.
        self.y_train : array
            y train set (classification labels).
        self.y_test : array
            y test set (classification labels).
        """
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            ...
            test_size=test_size,
            shuffle=True,
            stratify=self.output_values,  # Ensure stratified sampling for classification
            random_state=42
        )
    ...
    def split_train_validation_test(self, test_size=0.1, val_size=0.1, random_state_=42):
        ...
        test_size : float
            Proportion of the dataset to include in the test split (default = 0.1).
        val_size : float
            Proportion of the training dataset to include in the validation split (default = 0.1).
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            ...
            test_size=test_size,
            shuffle=True,
            stratify=self.output_values,  # Stratify by class labels
            random_state=random_state_
        )
        self.X_test, self.y_test = X_test, y_test
        ...
        self.X_train, self.X_val, self.y_train, self.y_val = train_test_split(
            ...
            test_size=val_size,
            shuffle=True,
            stratify=y_train,  # Stratify validation set
            random_state=random_state_
        )

    def yLabel_encoding(self, save_encoder=True):
        """
        Encode target labels with value between 0 and n_classes-1 using sklearn's LabelEncoder.

        This method fits a LabelEncoder to the training target values (y_train) and transforms
        both the training and test target sets. It saves the trained encoder as a pickle file for reuse.

        Parameters
        ----------
        save_encoder : bool, optional
            Whether to save the trained LabelEncoder to a file (default is True).
        ...
        self.y_test : np.ndarray
            Encoded test labels.
        self.target_classes : np.ndarray
            Classes found by the LabelEncoder.
        ...
        ------
        ValueError
            If y_train or y_test is None or not provided.
        """

        if self.y_train is None or self.y_test is None:
            raise ValueError("y_train and y_test must not be None for label encoding.")

        encoder = preprocessing.LabelEncoder()
        self.y_train = encoder.fit_transform(self.y_train)
        self.y_test = encoder.transform(self.y_test)
        self.target_classes = encoder.classes_
        self.encoder = encoder

        # # File path construction with flexibility for saving
        # if save_encoder:
        #     encoder_file = f"LabelEncoder_{self.trained_ML_model_name}_{self.output_names}_feature_improved_{self.is_feature_improved}_{self.tuning}_tuned.pkl"
        #     le_filepath = os.path.join('../outputs/trained_models/', encoder_file)
        #
        #     # Save the encoder
        #     with open(le_filepath, 'wb') as file:
        #         pk.dump(encoder, file)
        #     print(f"Label encoder saved at: {le_filepath}")
    def normalize_X_column_wise(self,
                                output_indicator='Trained_Normalizer_X_ColWise',
                                X_train=None,
                                X_val=None,
