...
def filter_dataset_range(dataset, filter_params):
    ...
    for param, bounds in filter_params.items():
        ...
        filtered_df = filtered_df[
            (filtered_df[param] >= lower_bound) & (filtered_df[param] <= upper_bound)
        ]
...
def interpolate_df(dataset, predicted_targets_dic, print_results_=False):
    """
    ...
    met0, met1 = find_nearest_top_bottom(
        predicted_targets_dic["metallicity"], my_list_met
    )
    ...
    df_to_interpolate = filter_dataset_range(dataset, filter_params).reset_index(
        drop=True
    )
    ...
    df_interpolated_ = pd.DataFrame(
        columns=df_to_interpolate.drop(
        ...
        for grav in my_list_g:
            for met in my_list_met:
                for c_o in range(0, len(my_list_c_o) - 1):
                    ...
                    df_to_interpolate_ = filter_dataset_range(
                        dataset, filter_params
                    ).reset_index(
                        drop=True
                    )  # .drop_duplicates(subset=['gravity', 'temperature', 'c_o_ratio', 'metallicity'])
                    ...
                    xi_mesh, yi_mesh = np.meshgrid(
                        xi, yi, indexing="ij"
                    )  # Meshgrid for interpolation

                    # Perform interpolation
                    df_interpolated_ = pd.DataFrame(
                        interp_func((yi_mesh, xi_mesh)).T,
                    ...
                    df_interpolated_all = pd.concat(
                        [df_interpolated_, df_interpolated_all], ignore_index=True
                    )
    ...
    df_interpolated_ = pd.DataFrame(
        columns=df_interpolated_all2.drop(
            columns=[
                "gravity",
                "temperature",
                "c_o_ratio",
        ...
        for temp in my_list_T:
            for grav in my_list_g:
                for met in range(0, len(my_list_met) - 1):
                    ...
                    df_to_interpolate_ = filter_dataset_range(
                        df_interpolated_all2, filter_params
                    ).reset_index(
                        drop=True
                    )  # .drop_duplicates(subset=['gravity', 'temperature', 'c_o_ratio', 'metallicity'])
        ...
        for met in my_list_met:
            for temp in my_list_T:
                for grav in range(0, len(my_list_g) - 1):
                    ...
                    filter_params = {
                        "gravity": (my_list_g[grav], my_list_g[grav + 1]),
                        "temperature": (temp, temp),
                        "c_o_ratio": (c_o, c_o),
                        "metallicity": (met, met),
                    }
...
def filter_dataframe(training_datasets, predicted_targets_dic):
    ...
    for col, value in predicted_targets_dic.items():
        if value not in filtered_df[col].values:
            nearest_value = filtered_df[col].values[
                np.argmin(np.abs(filtered_df[col].values - value))
            ]
...
def plot_boxplot_hist(data, x_label, xy_loc):
    ...
    plt.annotate(
        f"{x_label}=" + str(np.round(mean, 2)) + "$\pm$" + str(np.round(std, 2)),
        fontsize=11,
        xy=(xy_loc[0], xy_loc[1]),
...
def plot_spectra_errorbar(
    ...
    p = figure(
        title=(
            f"{object_name}: Calibrated Observational Spectra"
    ...
    if data_type == "x_y_yerr":
        ...
        lower = np.maximum(
            y_obs - y_obs_err, y_obs - y_obs * max_error_threshold
        )  # Sample data
...
def plot_pred_vs_obs_errorbar(
    ...
    lower = np.maximum(
        y_obs - y_obs_error, y_obs - y_obs * max_error_threshold
    )  # Sample data
    ...
    p = figure(
        title=f"{object_name}: Observational vs. ML Predicted Spectra",
        x_axis_label="Wavelength [𝜇m]",
    ...
    if __print_results__:
        print(
            "------------- Interpolated spectrum based on the ML predicted targets --------------"
        )
        print(filtered_df)
    ...
    p.line(
        ...
        + ", ".join(
            [
                ["log𝑔= ", "C/O= ", "[M/H]= ", "T= "][i] + str(np.round(ypred[i], 2))
                for i in range(4)
            ]
        ),
    )
...
def plot_pred_vs_obs_errorbar_stat_bokeh(
    stat_df,
    confidence_level,
    object_name,
    x_obs,
    ...
    p = figure(
        title=object_name
        + ": Observational vs. ML Predicted Spectra"
        + " [𝛘2="
        + str(chi2_stat)
        + "]",
    ...
    p.line(
        ...
        + ", ".join(
            [
                ["log𝑔= ", "C/O= ", "[M/H]= ", "T= "][i]
                + str(np.round(list(predicted_targets_dic.values())[i], 2))
                for i in range(4)
            ]
        )
        + ", R="
        + str(np.round(radius, 2))
        + " Rjup",
    )
...
def calculate_confidence_intervals_std_df(
    dataset_df,
    __print_results__=False,
    ...
    stat_df = pd.DataFrame(
        columns=["confidence_level_lower", "confidence_level_upper"], index=None
    )
    ...
    if __plot_calculate_confidence_intervals_std_df__:
        ...
        p.line(
            ...
            line_width=2,
            legend_label="Mean",
        )  # +', '.join([['log𝑔= ','C/O= ', '[M/H]= ', 'T= '][i]+str(np.round(ypred[i],2)) for i in  range(4)])))
...
def plot_with_errorbars(
    x_obs, y_obs, err_obs, x_pre, y_pre, err_pre, title="Data with Error Bars"
):
    """
    ...
    source_obs = ColumnDataSource(
        data=dict(
            x_obs=x_obs,
            y_obs=y_obs,
            upper_err_obs=upper_err_obs,
            lower_err_obs=lower_err_obs,
        )
    )
    source_pre = ColumnDataSource(
        data=dict(
            x_pre=x_pre,
            y_pre=y_pre,
            upper_err_pre=upper_err_pre,
            lower_err_pre=lower_err_pre,
        )
    )
...
def chi_square_test(
    ...
    if len(data1) != len(data2):
        f1 = interp1d(
            x_obs, data1, kind="cubic", fill_value="extrapolate"
        )  # interp1d(x1, data1, kind='quadratic')
    ...
    chi2_stat = np.round(
        np.sum(((data1 - data2) / np.sqrt(error1**2 + error2**2)) ** 2), 2
    )
    # print(data1,data2)
    ...
    if __print_results__:
        print(
            f"Radius = {'{:.2f}'.format(radius)} R_Jup:  𝛘2 = {chi2_stat}, p-value = {p_value}"
        )

    return chi2_stat, p_value


def plot_chi_square_p_value(
    radius,
    chi_square_list,
    p_value_list,
):
    ...
    source = ColumnDataSource(
        data=dict(
            radius=radius, chi_square_list=chi_square_list, p_value_list=p_value_list
        )
    )
    ...
    fig = figure(
        width=800,
        height=400,
        title="Chi-square and p-value",
        x_axis_label="Radius [R_Jup]",
...
def plot_filtered_spectra(
    ...
    filtered_df2 = filtered_df.sort_values(feature_to_plot, ascending=True).drop(
    ...
    )  # Number of colors needed (excluding x-axis)
    ...
    cbar = plt.colorbar(
        plt.cm.ScalarMappable(
            cmap=cmap,
            norm=plt.Normalize(
                vmin=filter_bounds[feature_to_plot][0],
    ...
    if __save_plots__:
        plt.savefig(
            os.path.join(
                __reference_data__,
                "figures",
                feature_to_plot + "_training_examples.pdf",
            ),
            dpi=500,
            bbox_inches="tight",
        )
...
def plot_ML_model_loss_bokeh(trained_ML_model_history=None, title=None):
    ...
    p.line(
        epochs,
        trained_ML_model_history["val_loss"],
        line_color=colors[0],
        line_dash="dotted",
        line_width=2,
    )
    ...
    p.line(
        epochs,
        trained_ML_model_history["output__gravity_loss"],
        line_color=colors[1],
        line_dash="solid",
        line_width=2,
    ...
    p.line(
        epochs,
        trained_ML_model_history["val_output__gravity_loss"],
        line_color=colors[1],
        line_dash="dotted",
        line_width=2,
    )
    ...
    p.line(
        epochs,
        trained_ML_model_history["output__c_o_ratio_loss"],
        line_color=colors[2],
        line_dash="solid",
        line_width=2,
    ...
    p.line(
        epochs,
        trained_ML_model_history["val_output__c_o_ratio_loss"],
        line_color=colors[2],
        line_dash="dotted",
        line_width=2,
    )
    ...
    p.line(
        epochs,
        trained_ML_model_history["output__metallicity_loss"],
        line_color=colors[3],
        line_dash="solid",
        line_width=2,
    ...
    p.line(
        epochs,
        trained_ML_model_history["val_output__metallicity_loss"],
        line_color=colors[3],
        line_dash="dotted",
        line_width=2,
    )
    ...
    p.line(
        epochs,
        trained_ML_model_history["output__temperature_loss"],
        line_color=colors[4],
        line_dash="solid",
        line_width=2,
    ...
    p.line(
        epochs,
        trained_ML_model_history["val_output__temperature_loss"],
        line_color=colors[4],
        line_dash="dotted",
        line_width=2,
    )
...
def plot_ML_model_loss_bokeh_with_baseline(
    trained_ML_model_history=None, base_model_history=None, title=None
):
    """
    Plot the trained model history against baseline model history
    """
    ...
    p.line(
        epochs,
        base_model_history["loss"],
        line_color=colors[1],
        line_dash="solid",
        line_width=2,
        legend_label="Total loss - base",
    )
    p.line(
        epochs,
        base_model_history["val_loss"],
        line_color=colors[1],
        line_dash="dotted",
        line_width=2,
        legend_label="Validation loss - base",
    )
...
def plot_ML_model_loss_plt(
    trained_ML_model_history=None,
    title=None,
    __reference_data__=None,
    __save_plots__=False,
):
    ...
    if __save_plots__:
        plt.savefig(
            os.path.join(__reference_data__, "figures", "Trained_CNN_Huber_Loss.pdf"),
            dpi=500,
            bbox_inches="tight",
        )
...
def plot_boxplot(
    ...
    if __save_plots__:
        plt.savefig(
            os.path.join(__reference_data__, "figures", saved_file_name + "_.pdf"),
            dpi=500,
            bbox_inches="tight",
        )
...
def plot_tricontour_chi2_radius(
    tuned_ML_R_param_df,
    literature_info,
    ...
    for target in list_:
        ...
        #                                                          y=.95, fontsize=15)
        plt.suptitle(
            f"$\chi_{{min}}^2$={round(min_Z, 1)}"
            + r", $R_{\chi^2_\mathrm{min}}$="
            + f"{round(min_Y, 2)}, "
            + target_dict[target]
            + f"= {round(min_X, 2)}$\pm${Tstd}",
            y=0.95,
            fontsize=15,
        )
...
def plot_pred_vs_obs_errorbar_stat_matplotlib(
    stat_df,
    confidence_level,
    object_name,
    x_obs,
    ...
    ax.set_title(
        f"{object_name}: Observational vs. ML Predicted Spectra"
        + " [$\chi^2$="
        + str(chi2_stat)
        + "]",
        fontsize=16,
    ...
    if __save_plots__:
        plt.savefig(
            os.path.join(
                __reference_data__,
                "figures",
                f"{object_name}_obervational_vs_MLpredicted_plt.pdf",
            ),
            dpi=500,
            bbox_inches="tight",
        )
...
def plot_regression_report(
    ...
    y_pred_train_list = (
        trained_DataProcessor.standardize_y_ColumnWise.inverse_transform(y_pred_train)
    )
    y_pred_train_list[:, 3] = 10 ** y_pred_train_list[:, 3]

    y_act_train_list = trained_DataProcessor.standardize_y_ColumnWise.inverse_transform(
        ytrain
    )
    ...
    y_pred_test_list = trained_DataProcessor.standardize_y_ColumnWise.inverse_transform(
        y_pred_test
    )
    y_pred_test_list[:, 3] = 10 ** y_pred_test_list[:, 3]

    y_act_test_list = trained_DataProcessor.standardize_y_ColumnWise.inverse_transform(
        ytest
    )
    ...
    for i in range(0, target_i):
        ...
        if __print_results__:
            print("\n\n----------------------- Test ------------------------")
            print(
                "R2: {:2.2f} \t  RMSE: {:2.2f} \t Mean+/-STD: {:2.2f}+/-{:2.2f}".format(
        ...
        axs[0].set_xlim(
            (-(abs(mean_train) + 3 * std_train), (abs(mean_train) + 3 * std_train))
        )
        ...
        axs[1].annotate(
            ...
            fontsize=11,
            xy=(xy_bottom[0], xy_bottom[1] + 0.06),
            xycoords="axes fraction",
        )
        axs[1].annotate(
            r"R$^2_{\rm test}$ =" + f'{"%0.2f" % r2_score_test}',
            fontsize=11,
            xy=(xy_bottom[0], xy_bottom[1] - 0.06),
            xycoords="axes fraction",
        )
        ...
        if __save_plots__:
            ...
            plt.savefig(
                os.path.join(
                    __reference_data__,
                    "figures",
                    f"regression_report_{target_name}.pdf",
                ),
                dpi=500,
                bbox_inches="tight",
            )
